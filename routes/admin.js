...
router.post('/categorias/nova',eAdmin,(req,res)=>{
    ...
    if(!req.body.slug || typeof req.body.slug == undefined || req.body.slug == null){
        erros.push({texto: "Noticia invalida"})
    }
    ...
    if(erros.length > 0){
        ...
        new Categoria(novaCategoria).save().then(()=>{
            req.flash("success_msg","Noticia salva com sucesso")
            res.redirect("/admin/categorias")
        }).catch((err)=>{
            req.flash("error_msg","Houve um erro ao salvar a noticia, tente novamente")
            res.redirect("/admin")
        })
    }
...
router.post("/categorias/edit",eAdmin,(req,res)=>{
    Categoria.findOne({_id: req.body.id}).then((categoria)=>{
        ...
        categoria.save().then(()=>{
            ...
            req.flash("error_msg", "houve um erro interno ao salvar a edição da noticia")
            res.redirect("/admin/categorias")
        })
...
router.post("/categorias/deletar",eAdmin,(req, res)=>{
    Categoria.remove({_id: req.body.id}).lean().then(()=>{
        ...
        req.flash("error_msg", "houve um erro deletar a noticia")
        res.redirect("/admin/categorias")
    })
})
